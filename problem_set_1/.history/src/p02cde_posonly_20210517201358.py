...
def main(train_path, valid_path, test_path, pred_path):
    ...
    print("The accuracy on valid set(y) is: ", np.mean(logreg.predict(x_val) == t_val))
    np.savetxt(pred_path_d, logreg.predict(x_val))

    v_plus = x_val[y_val==1]
    alphi = h(logreg.theta, v_plus).mean()
    print("The accuracy on valid set(modify) is: ", np.mean(predict(x_val) == t_val))
    ...
    def predict(x):
        return h(logreg.theta, x) / alphi >= 0.5
    # *** END CODER HERE
